...
def ensure_default_server(conn: sqlite3.Connection) -> None:
    ...
    cur = conn.execute("SELECT id FROM servers WHERE name = ?", ("Main Room",))
    row = cur.fetchone()
    if row is None:
        conn.execute("INSERT INTO servers(name) VALUES(?)", ("Main Room",))
        conn.commit()
...
def get_server_by_id(conn: sqlite3.Connection, server_id: int) -> Optional[dict]:
    cur = conn.execute("SELECT id, name FROM servers WHERE id = ?", (server_id,))
    row = cur.fetchone()
    if row is None:
        return None
